...
class CountDownViewModel : ViewModel() {
    ...
    private val _currentTimeString = MediatorLiveData<String>()
    val currentTimeString: LiveData<String> get() = _currentTimeString

    init {
        _currentTimeString.addSource(currentTime) { time ->
            _currentTimeString.value = DateUtils.formatElapsedTime(time / 1000)
        }
    }
    ...
    fun setInitialTime(minuteFocus: Long) {
        val initialTimeMillis: Long = minuteFocus * 60 * 1000
